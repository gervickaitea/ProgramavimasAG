...
int main()
{
        ...
        while (true)
        {
            ...
            if (!Entry.empty())
            {
                ...
                if (Entry.length() - 1 > Position && Raides(Entry) == true)
                {
                    ...
                    LongestName = max(LongestName, StudentObject.Vardas.length());
                    
                    StudentObject.Pavarde = Entry.substr(Position + 1);;
                    LongestSurname = max(LongestSurname, StudentObject.Pavarde.length());
                    
                    NewLine();

                    cout << "Kaip bus pateikiami studento namu darbu rezultatai?\nSpauskite 1, jei rezultatus ivesite Jus,\n2, jei norite, jog duomenys butu sugeneruoti atsitiktinai." << endl;
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (list<Studentas>::iterator IT = Failed.begin(); IT != Failed.end(); IT++)
                    {
                        IrasymasKvaili << (*IT).Vardas << string(LongestName + 1 - (*IT).Vardas.size(), ' ');
                        IrasymasKvaili << (*IT).Pavarde << string(LongestSurname + 1 - (*IT).Pavarde.size(), ' ');
                    ...
                    {
                        IrasymasProtingi << (*IT).Vardas << string(LongestName + 1 - (*IT).Vardas.size(), ' ');
                        IrasymasProtingi << (*IT).Pavarde << string(LongestSurname + 1 - (*IT).Pavarde.size(), ' ');
    ...
    else if (EntryChoice == 2)
    {
        ...
        while (getline(ReadFile, EntryLine))
        {
            ...
            if (/*AllLetters(Name) && AllLetters(Surname) &&*/ Skaiciai(Egzaminas))
            {
