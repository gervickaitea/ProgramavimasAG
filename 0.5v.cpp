...

{
	...
	for (int i = 0; i < vargs; i++) {
		...
		if (get(vargsiukai, i).GP < 5.00) {
			vargs_failas << get(vargsiukai, i).Vard << std::setw(20) << get(vargsiukai, i).Pav << std::setw(18) << get(vargsiukai, i).GP << std::endl;
		}
	}
	...
	for (int j = 0; j < kiek; j++) {
		...
		//int _gp = get(studentai, j).GP;
		if (get(studentai, j).GP >= paz) {
			prot_failas << get(studentai, j).Vard << std::setw(20) << get(studentai, j).Pav << std::setw(18) << get(studentai, j).GP << std::endl;
		}
	}
