...
void count_median(std::vector<int> pazymiai) //funkcija suskaiciuoja mediana
{
    int counter = 0;
...
void automated_marks_input(duomenys Eil[], int i, int how_many_marks) //funckija automatiskai suveda pazymius ir egz. pazymi
{
    ...
    {
        Eil[i].paz.push_back(generate_random_mark());
    }
...
void input_by_hand(duomenys Eil[], int i) //funkcija duoda vartotojui suvest pazymius ir egzamino pazymi
{
    int temp;
    do {
        std::cout << "Iveskite studento EGZ:\n";
        std::cin >> Eil[i].egz;
    ...
    do {
        ...
        if (Eil[i].paz.at(counter) != -1) { Eil[i].GP = Eil[i].GP + (float)Eil[i].paz.at(counter); }
        counter++;
    } while (Eil[i].paz.at(counter) != -1);
    counter--;
